...
NB_MH <- paste(
  "NB: L'indice di salute mentale è una misura di disagio psicologico (psychological distress)
  ottenuta dalla sintesi dei punteggi individuali riferiti a 4 dimensioni della salute mentale (ansia, depressione, perdita di controllo comportamentale o emozionale e benessere psicologico)."
)
ETA_PLOT <- "14-19"
ANNO_ULTIMO <- 2025
...
# 2. Indice di salute mentale 14-19 per sesso, 2016-2025 -------------------
salute_mentale_prep <- bes_giovani_eta_sesso |>
  filter(indicatore_breve == "salute_mentale", eta == ETA_PLOT) |>
...
plot_salute_mentale_14_19 <- salute_mentale_prep |>
  ...
  annotate("text", x = LOCKDOWN[1] - 0.6, y = 80, label = "pandemia",
           hjust = 1, vjust = 1, size = 3.5, color = grey_extra_sc) +
  ...
  labs(
    title = str_wrap(glue("Salute mentale dei 14-19enni in Italia (trend 2016-{ANNO_ULTIMO})"), 55),
...
plot_salute_mentale_14_19

# 3. Soddisfazione per le relazioni amicali 14-19 per sesso, 2005-2025 -----
amici_prep <- bes_giovani_eta_sesso |>
  filter(indicatore_breve == "amici", eta == ETA_PLOT) |>
  mutate(sesso = factor(sesso, levels = names(COL_SESSO)),
         quota = valore / 100,
...
plot_amici_14_19 <- amici_prep |>
  ggplot(aes(x = anno, y = quota, color = sesso, group = sesso)) +
  annotate("rect", xmin = LOCKDOWN[1] - 0.5, xmax = LOCKDOWN[2] + 0.5,
           ymin = -Inf, ymax = Inf, fill = "grey85", alpha = 0.5) +
  annotate("text", x = LOCKDOWN[1] - 0.6, y = 0.55, label = "pandemia",
           hjust = 1, vjust = 1, size = 3.5, color = grey_extra_sc) +
  geom_line_interactive(aes(tooltip = sesso, data_id = sesso), linewidth = rel(1.2)) +
  geom_point_interactive(aes(tooltip = glue("{sesso} {anno}: {etichetta}"), data_id = sesso), size = 2) +
  scale_x_continuous(breaks = seq(2005, ANNO_ULTIMO, by = 1)) +
  scale_y_continuous(labels = function(x) scales::percent(x, accuracy = 1), limits = c(0.25, 0.55)) +
  scale_color_manual(values = COL_SESSO) +
  f_theme_scuola() +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5),
        plot.caption = ggtext::element_textbox_simple(size = rel(0.65), lineheight = 1.1, margin = margin(t = 8)),
        plot.caption.position = "plot") +
  labs(
    title = str_wrap(glue("Soddisfazione per le relazioni con gli amici dei 14-19enni in Italia (trend 2005-{ANNO_ULTIMO})"), 55),
    subtitle = "In % dei 14-19enni, per sesso. Stima campionaria: leggere il trend, non i decimali",
